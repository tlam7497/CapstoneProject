...
def worker(filename):
    ...
    if os.path.exists('sentiment_amazon_category/done/'+ wfile +'.tsv'):
        return
    with open(filename,'r') as f:
        next(f)
        pending = []
        with open('sentiment_amazon_category/sentiment_amazon_category_'+ wfile +'.tsv', 'w') as g: 
    ...
    with open('sentiment_amazon_category/done/'+ wfile +'.tsv', 'w') as h : # Create a folder to mark a file is cleaned 
        h.write('done')
...
jobs = ['sentiment_amazon_clean/sentiment_amazon_cleaned_{}.tsv'.format(k) for k in range(starting, stopping)]
