...
def worker(filename):
    ...
    if os.path.exists('sentiment_gr_clean/done/'+ wfile +'.tsv'):
        return
    wnl = WordNetLemmatizer()
    with open(filename,'r') as f:
        next(f)
        pending = []
        with open('sentiment_gr_clean/sentiment_gr_cleaned_'+ wfile +'.tsv', 'w') as g: 
            pending.append('book_id' + '\t' + 'overall'+ '\t'+'reviewTime' + '\t' + 'asin' + '\t' + 'reviewText' + '\t' + 'cleaned_text' + '\n')
    ...
    with open('sentiment_gr_clean/done/'+ wfile +'.tsv', 'w') as h : # Create a folder to mark a file is cleaned 
        h.write('done')
...
jobs = ['goodreads_official/goodreads_official_{}.tsv'.format(k) for k in range(starting, stopping)]
results = pool.map(worker, jobs)
